...
npm run build          # プロダクションビルド（out/ + インストーラー生成）
                       #   Windows: dist/*.exe（NSIS）/ Linux: dist/*.AppImage + dist/*.deb
...
- `ipc/filesystem.ts` の `SHOW_IN_FOLDER` は Linux では `wslpath` + `explorer.exe`（WSL2）
  または `xdg-open`（通常 Linux）にフォールバックする
...
パス結合は `joinPath(dir, file)` ヘルパーを使用（Windows: `\` / Linux: `/` を自動選択）。
絵文字フォント（`Noto Color Emoji` 等）は `.drop-icon` / `.banner-icon` にのみスコープして
テキストの文字間隔崩れを防止している。
...
- **CI** (`ci.yml`): `main` への push と全 PR でテスト実行。`concurrency` で重複実行をキャンセル
  - `paths` フィルターにより、ドキュメント（README.md 等）のみの変更では CI をスキップ
  - CI が実行されるパス: `src/**`, `tests/**`, `scripts/**`, `package.json`,
    `package-lock.json`, `*.config.*`, `tsconfig*.json`, `.github/workflows/ci.yml`
- **Release** (`release.yml`): `v*.*.*` タグ push でトリガー。Windows・Linux をビルドし
  `dist/*.exe`（Windows）・`dist/*.AppImage`・`dist/*.deb`（Linux）を GitHub Release に添付
